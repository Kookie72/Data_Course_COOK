...
#The mean-squared-error of a model. The smaller, the better.
mean(mod1$residuals^2) ## [1] 9.911209
mean(mod2$residuals^2) ## [1] 29.02048
...
mtcars %>% 
  ...
  theme_minimal() +
  annotate("text",x=250,y=32,label=deparse(mod1$call)) + # deparse() converts "call" into a readable text string
  annotate("text",x=250,y=30,label=deparse(mod2$call)) +
  annotate("text",x=250,y=28,label=deparse(mod3$call))
